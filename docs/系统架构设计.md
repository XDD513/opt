# 中医体质辨识与健康管理系统 - 系统架构设计文档

> **文档版本**: v1.0.0  
> **创建日期**: 2025-01-XX  
> **最后更新**: 2025-01-XX

---

## 1. 总体概述

系统采用“前后端分离 + 分层架构 + 缓存/消息中间件”的模式构建，核心思想是：前端专注交互体验，后端负责业务编排，数据和中间件提供可靠支撑，配合配置中心与持续交付体系，保证可扩展、可维护、可观测。

关键层次：

- **客户端层**：基于 Vue3 的 Web SPA，同时兼容 H5/桌面浏览器，后续可复用逻辑产出小程序/APP。
- **接入层**：Nginx 承担静态资源分发、HTTPS 终端、反向代理；**当前默认**与前端静态资源同处于 **Docker 前端容器** 内（见 `deploy/docker-compose.yml`）。CDN、多实例 Nginx 负载均衡可在访问量上升后按需引入。
- **应用层**：Spring Boot 应用，提供业务逻辑编排、多模态算法融合、DeepSeek LLM 接入。
- **AI 服务层**：基于 Python/Flask 的独立微服务，部署 YOLOv8 模型，负责舌象特征检测。
- **数据与中间件层**：MySQL（持久化）、Redis（二级缓存/分布式锁）、RabbitMQ（异步解耦）、Elasticsearch（全局检索）、OSS（云存储）。

---

## 2. 技术选型

| 层次 | 技术栈 | 说明 |
|------|--------|------|
| 前端 | Vue 3 + Vite + Element Plus + Pinia + ECharts | 现代 Web 开发框架、响应式布局 |
| 后端 | Spring Boot 2.7.18 + MyBatis-Plus + Spring Security | 核心业务架构、ORM 框架、权限安全 |
| AI 算法 | Python + Flask + YOLOv8 + ONNX | 舌象特征识别、高性能推理 |
| LLM | DeepSeek API | 智能健康咨询、体质报告解读 |
| 检索 | Elasticsearch 7.17 | 全局关键词搜索、分词检索 |
| 数据 | MySQL 8.0 + Redis 7（Docker 镜像 `redis:7-alpine`） | 持久化存储与高性能缓存 |
| 消息 | RabbitMQ + WebSocket | 异步通知与实时医患互动 |
| 运维 | Nacos + Docker | 配置管理与容器化部署 |

---

## 3. 后端分层架构

```
┌──────────────────────────────────────────────┐
│                 接入层 (Controller)          │
│  - REST API / WebSocket Endpoint             │
└──────────────────────────────────────────────┘
┌──────────────────────────────────────────────┐
│                    业务层                     │
│  - Service Interface & Impl                  │
│  - 多模态融合逻辑 (ExamService)               │
│  - 混合推荐算法 (RecommendationService)       │
└──────────────────────────────────────────────┘
┌──────────────────────────────────────────────┐
│                   数据访问层                  │
│  - MyBatis-Plus / ES Repository              │
│  - MapStruct (DTO/Entity 转换)               │
└──────────────────────────────────────────────┘
┌──────────────────────────────────────────────┐
│                 基础设施层                    │
│  - Security / JWT / Redis / MQ / OSS         │
│  - AI Service Client / DeepSeek Integration  │
└──────────────────────────────────────────────┘
```

---

## 4. 核心业务流程

### 4.1 智能化体质辨识流程 (Multi-modal Fusion)

1. **数据采集**：前端提交 66 题问卷得分及舌象照片。
2. **特征提取**：
   - 后端调用 **AI Service**，通过 **YOLOv8** 模型识别舌象特征（如 `chihenshe`、`pangdashe`）。
   - 后端计算问卷原始分及转化分。
3. **算法融合**：系统根据中医辨证规则，将舌象特征作为加权项修正问卷结果，得出最终体质。
4. **LLM 增强**：调用 **DeepSeek API**，基于体质结论与检测特征生成个性化健康解读。
5. **结果持久化**：存储识别记录、AI 分析文本及特征框坐标。

### 4.2 辨证施养推荐流程

1. **召回阶段**：根据用户主导/兼夹体质，从数据库召回匹配的药膳、穴位方案。
2. **排序阶段 (Weighted Ranking)**：
   - **特征加分**：若存在特定舌象特征，提升对应功效方案的权重。
   - **分众化调整**：根据用户年龄、地域属性调整排序。
3. **过滤阶段**：剔除禁忌项，保留 Top N 推荐结果展示。

### 4.3 内容体系（药膳/穴位/文章）

- 高频查询列表使用 Redis 缓存，命名空间示例：`hospital:common:recipe:list`。
- 热门搜索词使用 Redis Sorted Set 存储 `search:hot_keywords`。
- 富文本图片/封面统一存放在 OSS，通过后端生成带签名的访问 URL。

### 4.4 医患对话与通知

- 会话与消息落库：`conversation`、`conversation_message`。
- 消息发送后通过 RabbitMQ 事件异步广播；前端使用 WebSocket + STOMP 订阅 `/user/{userId}/queue/conversations` 获取实时消息。
- 离线/未读通过 `conversation.unread_for_*` 计数聚合；前端基于会话列表展示未读，并支持一键标记已读、按会话类型/参与方筛选（如前端提供筛选条件）。

---

## 5. 前端架构

```
hospital-frontend
├── src
│   ├── api/            # Axios 封装与模块化接口
│   ├── components/     # 通用组件（图表卡片、上传等）
│   ├── composables/    # 组合式函数 useAvatarUpload 等
│   ├── router/         # 路由与权限守卫，区分角色
│   ├── stores/         # Pinia 状态管理（user/admin）
│   ├── utils/          # searchHistory、auth 等工具
│   └── views/          # 业务页面（患者/管理员）
├── public/
└── vite.config.js
```

- **权限体系**：路由 `meta.roles` + Pinia + 路由守卫实现；Token 失效跳转登录。
- **UI 规范**：Element Plus + BEM 命名，统一色系；表格/图表等组件抽象复用。
- **可视化**：ECharts 统一封装，支持健康趋势、体质测试统计、会话未读等。
- **状态缓存**：Pinia + localStorage/sessionStorage，减少重复请求。

---

## 6. 数据流 & 时序

### 6.2 体质测试时序

```
Patient -> ConstitutionController -> ConstitutionService
        -> QuestionnaireMapper -> MySQL
        -> ResultBuilder -> Redis Cache
        -> Response -> Frontend (展示报告 + 推荐)
```

### 6.3 实时对话时序

```
Patient/Admin -> REST `/api/conversations/{conversationId}/messages` -> ConversationService
               -> ConversationMessageMapper -> MySQL (写入 `conversation_message`)
               -> ConversationService 刷新 `conversation` 快照/未读计数
               -> RabbitMQ -> ConversationMessageListener
               -> WebSocket(STOMP) -> 对端客户端(`/user/{userId}/queue/conversations`)
```

---

## 7. 缓存 & 消息策略

### 7.1 缓存键构建规范

系统统一使用 `CacheKeyBuilder` 工具类构建缓存键，避免硬编码和重复拼接：

```java
// 示例：构建用户列表缓存键
String cacheKey = CacheKeyBuilder.of(CacheConstants.USER_LIST_CACHE_PREFIX)
    .appendPage(page, pageSize)
    .appendParamsHash(params)
    .build();
```

**缓存键命名规范**：
- 格式：`{前缀}:{模块}:{类型}:{标识}`
- 示例：`hospital:admin:user:list:p1:s10:h:a1b2c3d4`
- 分页参数使用 `:p{page}:s{size}` 格式
- 复杂参数使用哈希值 `:h:{hash}` 压缩

### 7.2 缓存TTL策略

统一使用 `CacheTtlPolicy` 枚举管理过期时间：

| 策略 | 过期时间 | 适用场景 |
|------|----------|----------|
| `CONVERSATION_MESSAGE_LIST` | 1分钟 | 会话消息列表 |
| `CONVERSATION_LIST` | 2分钟 | 会话列表 |
| `CONVERSATION_DETAIL` | 5分钟 | 会话详情 |
| `DOCTOR_DETAIL` | 30分钟 | 医生信息 |
| `AI_RECOMMENDATION` | 30分钟 | AI推荐结果 |
| `CONSTITUTION_TYPE` | 24小时 | 体质类型字典 |

### 7.3 缓存场景与降级

| 场景 | 缓存 Key 示例 | 过期时间 | 降级策略 |
|------|---------------|----------|----------|
| 文章/药膳列表 | `hospital:common:article:list` | 15-30 分钟 | 直接查询 DB，叠加限流 |
| 体质测试结果 | `hospital:constitution:result:{userId}` | 30 分钟 | 重新计算 |
| 热门搜索词 | `search:hot_keywords` (ZSet) | 永久 | 返回预置词库 |
| 会话头像签名 | `hospital:conversation:avatar:{id}` | 5 分钟 | 使用非签名 URL |

**消息通知**：

- RabbitMQ：会话消息事件异步广播（由 `ConversationMessageListener` 消费并推送）。
- WebSocket：实时消息推送；离线未读通过 `conversation.unread_for_*` 计数聚合，前端在会话列表/历史中展示并可标记已读。

---

## 8. 配置与部署架构

### 8.1 配置中心（Nacos）

- `hospital-appointment-system-dev.yaml`：数据库、Redis、JWT、OSS、MQ、日志、跨域等。
- `hospital-frontend-dev.yaml`：API Base URL、OSS 直传、主题配置等。
- 区分命名空间（dev/staging/prod），变更支持灰度与回滚。

### 8.2 部署拓扑（当前：单机 Docker Compose）

**现状说明**：生产/联调环境可采用 **单台云服务器（或物理主机）**，在主机上安装 **Docker Engine**，使用仓库根目录下 **`deploy/docker-compose.yml`** 将 **前端（Nginx）、后端（Spring Boot）、AI 推理（Python）、MySQL、Redis、RabbitMQ、Elasticsearch、Nacos** 等以 **独立容器** 方式统一编排；各容器处于同一 **Compose 默认网络**，通过 **服务名** 互访（如 `backend` → `mysql`、`nacos`、`ai-service`），对外主要暴露 **80**（门户与反代）、按需暴露 **8000**（直连后端调试）等端口。

**逻辑拓扑**（与容器一一对应，非多机分布）：

```
[用户浏览器]
      |
 HTTP(S) :80
      |
+---------------- 单台主机 / Docker Compose ----------------+
|  [frontend 容器: Nginx + 静态 SPA]                          |
|         | 反向代理 /api → backend:8000                      |
|  [backend 容器: Spring Boot]                                |
|    → mysql:3306 | redis:6379 | nacos:8848                  |
|    → rabbitmq:5672 | elasticsearch:9200 | ai-service:5000 |
+------------------------------------------------------------+
```

- **与「多机分布式」表述的关系**：论文或对外说明中若曾写「前端 2 台、后端 4 台、数据库主从集群」等，**与当前仓库默认落地不一致**；应以 **本段及 `docs/uml/figure-deployment-single-host-docker.puml`** 为准。后续若并发与可用性要求提高，可再演进为多节点：增加后端副本 + 前置 SLB、MySQL 主从、Redis 哨兵/集群等。
- **对象存储 OSS**：业务上仍支持将静态资源、富文本图片等对接 **OSS**；若未单独部署 MinIO/OSS 容器，则与 Compose 无冲突，按配置启用即可。
- **UML 部署图**：见 `docs/uml/figure-deployment-single-host-docker.puml`（导出 PNG/SVG 可作论文插图）。

### 8.3 日志与监控

- Logback + JSON 输出，区分 INFO/ERROR，附带 traceId。
- OperationLog 注解记录关键操作，便于审计追踪。
- 建议集成 Prometheus + Grafana 或阿里云 ARMS 采集 JVM/接口指标。

---

## 9. 错误码规范

### 9.1 错误码分类

系统采用分层错误码体系，便于问题定位和分类处理：

| 范围 | 类型 | 说明 |
|------|------|------|
| 200-299 | 通用状态码 | 成功、系统错误等 |
| 400-499 | 客户端错误 | 参数错误、未授权、资源不存在等 |
| 500-599 | 服务器错误 | 内部错误、服务不可用等 |
| 1000-1099 | 用户相关 | 用户不存在、已存在、账号禁用等 |
| 1100-1199 | Token相关 | Token无效、过期、缺失等 |
| 1200-1299 | 认证与用户相关 | 用户不存在、不可用等 |
| 1300-1399 | 体质测试相关 | 测试记录不存在、未完成、类型不存在 |
| 1800-1899 | 体质测试相关 | 测试记录不存在、未完成、类型不存在 |
| 1900-1999 | 药膳相关 | 药膳不存在、已收藏、未收藏 |
| 2000-2099 | 数据库相关 | 查询、插入、更新、删除失败 |
| 2100-2199 | 缓存相关 | 缓存操作失败、键不存在 |
| 2200-2299 | 健康打卡相关 | 打卡记录不存在、今日已打卡 |
| 2300-2399 | 统计相关 | 统计数据查询失败 |
| 2400-2499 | 文件上传相关 | 上传失败、类型不支持、大小超限 |
| 2500-2599 | AI推荐相关 | 推荐失败、服务不可用 |

### 9.2 异常处理

`GlobalExceptionHandler` 统一处理异常，支持：
- **业务异常**：`BusinessException`，返回对应错误码和消息
- **参数校验异常**：`MethodArgumentNotValidException`，返回字段级错误信息
- **系统异常**：记录 traceId 便于追踪，返回友好错误提示

**日志格式**：
```
业务异常 [traceId={traceId}, method={method}, uri={uri}, code={code}, message={message}]
```

## 10. 安全设计

- **认证**：JWT + Spring Security，Token 存 Redis（黑名单机制），支持退出/失效。
- **授权**：角色粒度（患者/管理员） + 注解/表达式控制接口访问。
- **数据安全**：密码 Bcrypt 加密、头像 URL 清洗、敏感字段脱敏。
- **输入校验**：Hibernate Validator + 全局异常，避免脏数据。
- **防护措施**：CSRF 防护、XSS 过滤、SQL 预编译、接口限流/验证码。

---

## 11. DTO映射规范

系统使用 **MapStruct** 进行对象映射，替代手写 `BeanUtils.copyProperties`：

### 11.1 映射器定义

```java
@Mapper(componentModel = "spring")
public interface DtoMapper {
    // User实体转UserInfoResponse
    @Mapping(target = "adminId", ignore = true)
    UserInfoResponse toUserInfoResponse(User user);
    
    // 列表映射
    List<UserInfoResponse> toUserInfoResponseList(List<User> users);
    
    // 复杂映射（体质测试转统计DTO）
    @Mapping(source = "testDate", target = "date", dateFormat = "yyyy-MM-dd")
    StatisticsDTO.RecentTest toRecentTest(TestRecord test);
}
```

### 11.2 使用规范

- 所有 DTO 与 Entity 之间的转换统一使用 `DtoMapper`
- 忽略字段使用 `@Mapping(target = "field", ignore = true)`
- 日期格式转换使用 `dateFormat` 参数
- 复杂字段映射使用 `@Mapping(source = "src", target = "dst")`

## 12. 可扩展性与演进

| 方向 | 说明 |
|------|------|
| 微服务拆分 | 可按体质测试、内容、消息、统计等拆分服务，使用注册中心与 RPC。 |
| 多端扩展 | 基于组合式 API、服务抽象，快速延伸到小程序/APP。 |
| AI 能力 | 结合 AI 推荐缓存常量，后续可引入大模型做问答、推荐解释。 |
| 数据中台 | 统计模块开放统一数据 API，供 BI 与报表系统消费。 |

---

## 13. 风险与改进建议

| 风险 | 影响 | 建议 |
|------|------|------|
| WebSocket 扩容 | 消息丢失 | 本项目使用 RabbitMQ 做消息广播并由 `ConversationMessageListener` 推送，建议同样依赖 MQ 保障多实例同步 |
| 缓存穿透/击穿 | DB 压力 | 加布隆过滤器、热点预热、限流熔断 |
| 配置混乱 | 发布风险 | 所有敏感配置统一进 Nacos，启用加密与灰度策略 |
| 头像签名过期 | 图片无法显示 | 前端统一通过后端获取签名 URL，并在前端缓存/刷新 |

---

**文档结束**

